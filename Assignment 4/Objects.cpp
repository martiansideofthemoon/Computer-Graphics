...
void Frame::move_to(float x, float y, float z) {
  center[0] = x;
  center[1] = y;
  center[2] = z;
}

void Frame::set_direction(float x, float y, float z) {
  // Only taking 2D direction for now
  if (x > 0) {
    ry = 180 - 180*atan(z/x)/PI;
  } else {
    ry = -1*180*atan(z/x)/PI;
  }
}
...
float Handle::get_angle() {
  return ry;
}

void Handle::set_angle(float angle) {
  ry = angle;
}
